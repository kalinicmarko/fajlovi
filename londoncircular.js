var map;
		...
		function initMap() {
			...
			map.data.addListener('click', function(event) {
				...
				if(event.feature.getProperty('isColorful') == undefined){
					...
					document.getElementById("customTextArea").value = textareaText;
		...
		function populateTextArea()  {
			if (properties.length > 0) {
				...
				if (window.sessionStorage.getItem('plan') == "forSolus") {
					...
					document.getElementById("customTextArea").value += totalPriceSolus;
				}
				...
				if (window.sessionStorage.getItem('plan') == "forSharePlan")  {
					...
					document.getElementById("customTextArea").value += totalPriceShare;
				}
		...
		function resetMap(e) {
				initMap();
				var latlongChoice = document.getElementById(e.id).value;
				var latlongParts = latlongChoice.split(",");
				var newPos = new google.maps.LatLng(latlongParts[0], latlongParts[1]);
				var url = latlongParts[2];
				
				map.data.loadGeoJson(url);
				
				map.data.setStyle(function (feature) {
					// Bojamo označene oblasti zapamćene u session storage-u
					selectedAreas = JSON.parse(window.sessionStorage.getItem('properties')) || [];			  
					paintArea = selectedAreas.filter(function ( area ) {
						if (area.name === feature.getProperty('Name')) {
